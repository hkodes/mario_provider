...
class WalletList extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    if ((double.parse(walletTransation['open_balance'])).round() <
        (double.parse(walletTransation['close_balance'])).round()) {
      isPositive = false;
    } else
      isPositive = true;
    ...
    return Padding(
        padding: const EdgeInsets.only(top: 16.0),
        child: InkWell(
          onTap: showTransactionDialog(context, walletTransation),
          child: Container(
